...
import NewShowerForm from "../components/forms/NewShower";
...
const Shower: React.FC = () => {
  ...
  const fetchShowers = async (): Promise<void> => {
    const response = await apiService.fetchShowerRecords({
    ...
    if (response.data) {
      setShowers(response.data as ShowerRecord[]);
    }
  };
  const deleteShower = async (id: number): Promise<void> => {
    const conf = window.confirm("Sei sicuro di voler eliminare questa voce?");

    if (!conf) return;

    const response = await apiService.deleteShowerRecord(id);
  ...
  const [showers, setShowers] = useState<ShowerRecord[]>([]);
  const [formIsShown, setFormIsShown] = useState(false);
  const [editMode, setEditMode] = useState(false);
  const [editingShower, setEditingShower] = useState<ShowerRecord | null>(null);
    ...
    <div className="main-container">
      ...
      <div>
        <div className="header">
          <h1>Scheda docce di {guestFullName}</h1>
        ...
        {formIsShown && (
          ...
          >
            <NewShowerForm
              editData={editingShower || undefined}
