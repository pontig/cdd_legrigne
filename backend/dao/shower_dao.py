...
class ShowerDAO:
    ...
    def get_shower_entries(self, person_id: int) -> List[dict]:
        semester_constraint = " = %s" if session.get('semester') is not None else " IS NULL"
        query = f"""
            SELECT doccia.*, account.nome, account.cognome FROM doccia
            JOIN account ON doccia.firma = account.id
    ...
    def create_shower_entry(self, data: dict) -> None:
        """Create a new shower entry"""
        query = """
            INSERT INTO doccia VALUES
    ...
    def delete_shower_entry(self, entry_id: int) -> None:
        """Delete a shower entry"""
        query = "DELETE FROM doccia WHERE id = %s"
        connection = None
        cursor = None
